...
export const deleteEmail = async (req, res) => {
  try {
    ...
    if (emailDeleted) {
      return res.status(200).json({ message: "Email eliminado correctamente en todos los administradores" });
    }

    return res.status(404).json({ message: "Email no encontrado en ningún administrador" });
  } catch (error) {
